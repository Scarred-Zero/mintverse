...
def create_admin_page():
    ...
    if form_data.validate_on_submit():
        ...
        error_message = validate_password(password)  # Replace with your password validation function
        if error_message:
...
def approve_gasfee_deposit(deposit_id):
    ...
    if ether:
        ...
        ether = Ether(
            user_id=deposit.user_id, gas_fee_balance=deposit.eth_amount
        )
        db.session.add(ether)
...
def approve_minting(nft_id):
    ...
    try:
        ...
        pending_nft.status = NFTStatus.AVAILABLE # ✅ Keep NFT history while approving
        db.session.commit()
...
def add_user():
    ...
    if request.method == 'POST':
        ...
        if form_data.validate_on_submit():
            ...
            existing_user = User.query.filter_by(email=email).first()
            if existing_user:
                flash('User already exists. Please register a new user.', category='error')
                return redirect(url_for('admin.add_user'))
            ...
            try:
                ...
                db.session.commit()
                flash(f'You have successfully created an account for {name}!', category='success')
                flash('Remember to save the password either on the browser or somewhere else.', category='success')
    ...
    return render_template('admin/add-user.html', current_user=current_user, form=form_data,
...
def user_profile(usr_id):
    ...
    return render_template('admin/user-profile.html', user=user, usr_id=usr_id,
...
def edit_profile(usr_id):
    ...
    return render_template('admin/edit-profile.html', form=form_data, search_form=search_form, user=user,
...
def admin_nft_listing():
    try:
        ...
        print(f"An error occurred fetching Nfts from Database: {str(e)}")
        return redirect(url_for("admin.admin_dashboard", current_user=current_user))
...
def delete_all_requests():
    try:
        ...
        flash("All Minting Requests have been successfully deleted!", category="success")
