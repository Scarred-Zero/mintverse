...
class NFT(db.Model):
    ...
    status = db.Column(
        db.Enum(NFTStatus), nullable=False, default=NFTStatus.LISTED
    )  # ✅ Use Enum
    creator = db.Column(db.String(100), nullable=False)
    buyer = db.Column(db.String(100), nullable=True)
    buyer_id = db.Column(
        db.Integer,
        db.ForeignKey("users.id", name="fk_nft_buyer_id"),  # ✅ Explicitly name the foreign key constraint
        nullable=True,
    )
    user_id = db.Column(
        db.Integer,
        db.ForeignKey("users.id", ondelete="CASCADE", name="fk_nft_user"),  # ✅ Explicitly name the constraint
        nullable=False,
    )
    ...
    def data(self):
        ...
        display_status = (
            NFTStatus.SOLD.value
            if self.buyer_id == current_user.id
            else NFTStatus.AVAILABLE.value
        )
