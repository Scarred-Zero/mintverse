...
window.addEventListener('scroll', function() {
  ...
  if (windowTop > 50) {
      document.querySelector('.scroll-to-top').classList.add('reveal');
  } else {
      document.querySelector('.scroll-to-top').classList.remove('reveal');
  }
});
window.addEventListener('scroll', function() {
  var windowTop = window.scrollY + 1;
  ...
  if (windowTop > 50) {
      document.querySelector('.scroll-to-bottom').classList.add('reveal');
  } else {
      document.querySelector('.scroll-to-bottom').classList.remove('reveal');
  }
});

document.querySelectorAll('a.js-scroll-trigger').forEach(function(anchor) {
  anchor.addEventListener('click', function(e) {
      e.preventDefault();
      const href = anchor.getAttribute('href');
      const targetId = href.split('#')[1]; // Get the fragment part
      const target = document.getElementById(targetId); // Use getElementById instead of querySelector
      if (target) {
          console.log('Scrolling to:', target);
          window.scrollTo({
              bottom: target.offsetTop + 100,
              behavior: 'smooth'
          });
      } else {
...
const eyeButton = document.querySelector("#passIcon");
