...
def offers_page():
    ...
    offers = (
        Offers.query.filter_by(user_id=current_user.id)
        .order_by(Offers.timestamp.desc())
        .all()
    )
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if nft.price >= Decimal("2.0000"):
            min_allowed_price = max(
                nft.price - Decimal("1.0000"), Decimal("0.001")
            )  # ✅ Ensure minimum price is never below 0.001 ETH
        else:
            min_allowed_price = max(
                nft.price - Decimal("0.0200"), Decimal("0.001")
            )  # ✅ Adjust threshold for NFTs priced ≤ 1.9999 ETH
...
def create_nft_page():
    ...
    nft_hst = (
        db.session.query(
            ...
            db.case((NFT.id.isnot(None), NFT.status), else_=PendingNFTs.status).label(
                "status"
            ),
            PendingNFTs.user_id,
            db.case(
                (NFT.id.isnot(None), NFT.timestamp), else_=PendingNFTs.timestamp
            ).label("timestamp"),
            db.case((NFT.id.isnot(None), NFT.ref_number), else_=db.literal(None)).label(
                "ref_number"
            ),
            db.case((NFT.id.isnot(None), NFT.buyer), else_=db.literal(None)).label(
                "buyer"
            ),  # ✅ Updated Buyer Info
        )
...
def get_my_listed_collection():
    ...
    try:
        listed_nfts = NFT.query.filter(
            NFT.user_id == current_user.id, NFT.status == NFTStatus.AVAILABLE.value
        ).all()
